 ...
 *
 * @package Zamp_Request
 */

/**
 *    Handling XSS prevention.
 *
 *    disclaimer : some of the methods in this class are derived from CodeIgniter
 *
 * @category Zamp
 * @package Zamp_Request
 * @author Mathan kumar R <phpmathan@gmail.com>
 * @version SVN: $Id: Request.php 381 2018-12-25 13:09:18Z phpmathan $
 */
class Zamp_Request {
	/**
	 *    Character set
	...
	public function xssClean($str, $is_image = false) {
		...
		if((array) $str === $str) {
			while(list($key) = each($str)) {
				$str[$key] = $this->xssClean($str[$key], $is_image);
			}
		...
		$str = Zamp_Security::removeInvisibleCharacters($str);
	 ...
	 * @param    bool $xssClean [default: false]
	 * @return    string
	 */
	 ...
	 * @return array|string
	 */
	public function arrayXssClean($data, $is_image = false) {
		if((array) $data === $data) {
			$new_data = [];
			foreach($data as $key => $value)
				$new_data[$key] = ((array) $value === $value) ?$this->arrayXssClean($value, $is_image) :$this->xssClean($value, $is_image);
			
			return $new_data;
		}
		else
			return $this->xssClean($data, $is_image);
	}
	...
	public function get($name = null) {
		if($name !== null)
			return isset(Zamp::$core->_params['query'][$name]) ?Zamp::$core->_params['query'][$name] :null;
		else
			return Zamp::$core->_params['query'];
	}
	...
	public function setGet($name, $value) {
		Zamp::$core->_params['query'][$name] = $value;
	}
	 ...
	 * @access public
	 * @since v1.5.2
	 * @param bool $include_numeric_index [default: true]
	 * @param string $arg_separator [default: &]
	 * @param array $getData Input array to make url
	 * @return string
	 */
	public function getQueryUrl($include_numeric_index = true, $arg_separator = "&", $getData = '') {
		$getQueryUrl = '';
		...
		if((array) $getData === $getData) {
			...
			if($queryParams)
				$getQueryUrl .= '?'.http_build_query($queryParams, '', $arg_separator);
		}
